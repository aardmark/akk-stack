...
init-peq-editor: ##@init Initializes PEQ editor
	...
	$(DOCKER) exec peq-editor bash -c "git config --global --add safe.directory '*'; chown www-data:www-data -R /var/www/html && git -C /var/www/html pull 2> /dev/null || git clone https://github.com/aardmark/peqeditor.git /var/www/html && cd /var/www/html/ && cp config.php.dist config.php"
